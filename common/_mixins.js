...
export default {
	...
	methods: {
		...
		$requestSend(config) {
			...
			send_data['_token'] = _data.localData('im_token');
		...
		$socketSend(send_data, cb) {
			...
			((callback1, callback2) => {
				...
				if (send_data && _data.data('socket_state')) {
					...
					// console.log(44444)
					// console.log(_data.data('socket_url'))
					uni.connectSocket({
