	...
	<view class="">
		...
		<view class="padding bot_bg">
			...
			<view class="bg-white radius20 margin-top-sm" style="padding: 32rpx 41rpx;">
				...
				<view class="font26 col-999" style="margin-top: 32rpx;" v-for="item,index of order_sns" :key="index">
					订单编号：{{item}}
					<image @click="util.copy(item)" src="../../static/my/copy.png"
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			this.order_sns = e.id ? JSON.parse(e.id) : ''
			console.log(this.order_sns);
			e.id ? this.id = this.order_sns[this.order_sns.length-1] : '';
			this.getinfo();
			// 汇元快捷需开启绑定银行卡列表接口获取
			// this.getlist()
		},
