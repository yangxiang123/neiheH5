	...
	<view class="">
		...
		<view class="bg-white margin radius12 margin-top-sm" style="padding: 56rpx 40rpx;margin-top: 79rpx;">
			...
			<view class="margin-top-lg flex flex-wrap align-center">
				<view class="po-rela synthesisChild" style="margin-bottom:10rpx;" v-for="(item,index) in all.prize" :key="index">
		...
		<!-- <view class="text-center bg" style="padding-top: 20rpx;position: relative;">
			<image :src="all.cover"
				style="width: 415rpx;height: 415rpx;border-radius: 20rpx;border: solid 6rpx #19A2FF;" mode="aspectFill">
			</image>
		...
		<view class="flex align-center justify-center flex-wrap margin-top-lg">
			<view @click="hecheng" class="flex align-center justify-center"
				:class="merge_status != 1 ? 'bgbtn' : 'bgbtnwc text-white'" style="font-size: 48rpx;">
				{{merge_status != 1 ? '材料不足' : '立即合成'}}
			</view>
		...
		<view class="bg-white margin radius12 margin-top-sm" style="padding: 56rpx 40rpx;margin-top: 79rpx;">
			...
			<view class="margin-top-lg flex flex-wrap align-center">
				<view class="po-rela synthesisChild" style="margin-bottom:10rpx;" v-for="(item,index) in all.goods" :key="index">
					<view class="text-white flex align-center justify-center bfb">
						{{item.have_num}}/{{item.goods_num}}
					</view>
					<!-- <view class="hb" v-if="!item.compound_goods_status"></view> -->
					<image :src="item.goods_image"
						style="width: 181rpx;height: 181rpx;border-radius: 14rpx;border: solid 1rpx #FFB033;" mode="">
		...
		<view class="bg-white margin radius12 margin-top-sm" style="padding: 56rpx 40rpx;">
			...
			<view class="margin-top-lg" style="line-height: 46rpx;">
				<u-parse :html="all.description"></u-parse>
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			getinfo() {
				this.util.request('goods/colorful_merge_detail', {
					merge_id: this.id
				}).then(res => {
					this.all = res.info;
					this.merge_status = res.merge_status
					let _this = this;
					let date = new Date().getTime()
					let endTime = new Date(_this.all.end_time).getTime()
			...
			hecheng() {
				this.util.request('goods/do_colorful_merge', {
					merge_id: this.id
				}).then(res => {
...
<style>
	...
	.bgbtnwc {
		width: 661rpx;
		height: 148rpx;
		background: url('@/static/my/hcbtnbgwc.png') no-repeat;
		background-size: 100%;
		color: white;
	}
