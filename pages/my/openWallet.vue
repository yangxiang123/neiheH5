...
<script>
	...
	export default {
		...
		onLoad(e) {
			this.order.order_sn = e.order_sn
			this.order.update_time = e.update_time
			this.order.order_amount = e.order_amount	
		},
		methods:{
			getUserInfo() {
			...
			vip_pay(){
				...
				if(this.payType == -1){
					this.util.request('user/pay_wallet_order', {
					...
					this.util.request('user/pay_wallet_order', {
						...
						// 	})
						// },1000)
						if(res.response && res.response.redirect_url){
							// #ifdef H5
							window.location.href = res.response.redirect_url;
							// #endif
														
							// #ifdef APP-PLUS
							uni.setStorageSync('url', res.response.redirect_url);
							this.util.urlTo('../pay/pay');
							// #endif	
						}
			...
			getOrder(){
				// let order = {}
				// order.order_sn = this.$route.query.order_sn
				// order.update_time = this.$route.query.update_time
				// order.order_amount = this.$route.query.order_amount
				// this.order = order
			},
			getlist() {
