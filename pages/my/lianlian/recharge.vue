<template>
	<view>
		...
		<view class="recharge-box">
			<view class="rmb">¥</view>
			<view class="input">
				<u-input v-model="rechargeNum" placeholder="" :clearable="false" type="number" :focus="true"
					@input="inputChange" :custom-style="{'font-size':'80rpx','font-weight':700,'width':inputWidth}"
					:border="false" />
			</view>
			<view class="balance">
		...
		<view style="margin: 0 70rpx;position: fixed;top: calc(100vh - 200rpx); width: calc(100vw - 140rpx);">
			<u-button style="margin-top: 50rpx;" shape="circle" type="primary" @click="doRecharge">确定</u-button>
		</view>
...
<script>
import bank_listVue from './bank_list.vue'
	export default {
		data() {
			return {
				rechargeNum: '',
		...
		onLoad() {
			...
			let card = uni.getStorageSync('recharge')
			if(card && card != 1){
				this.bankCard = card
				//加载密码控件
			...
			uni.setStorageSync('recharge', false)
		...
		methods: {
			...
			doRecharge(){
				...
				if(!this.rechargeNum){
					this.util.showToast("请输入充值金额");
					return
				}
				let that = this
				//统一下单
				this.util.request('Lianlian/createOrder', {
					type:'USER_TOPUP',
					money:this.rechargeNum
				}).then(res => {
					that.txn_seqno = res.txn_seqno
					that.util.request('Lianlian/token', {
						...
						lianlianpay.invokePasswordControl({
							...
							passwordElementToken: res2.password_element_token, // 向服务端请求生成的 token
