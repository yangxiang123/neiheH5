          ...
          <text-scroll :text="notice" ref="scrollNotice"></text-scroll>
			...
			<view class="popup-layer" :class="popupLayerClass" @touchmove.stop.prevent="discard">
				...
				<view v-if="showtype" style="position: absolute; width: 100%; height: 100%;z-index: 21;">
					<view style="position: absolute;width: 150px;left:calc(50% - 75px); top: 40%; background-color: rgba(0, 0, 0, 0.5);border-radius:10px; text-align: center; padding:30upx 0">
...
<script>
	...
	export default {
		...
		onReady: function(res) {
			...
			uni.createSelectorQuery().select('.scroll-view').fields({scrollTop: true}, (res) => {
				console.log("res: ",res);
				return
			      if (res.scrollTop === 0) {
			        uni.pageScrollTo({
						...
						for (let j = 0; j < row.length; j++) {
							...
							if (EM.alt == item) {
								...
								let imgstr = '<img style="width:22px;height:22px;" src="' + _data.data(
									'static_url') + '/static/emoji/' + EM.url + '">';
